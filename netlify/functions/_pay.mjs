...
// are sourced from the SCA's balance (sourceAccount = agent SCA). Same-chain
// Arc->Arc, NO forwarder — this is the exact shape proven on-chain (0.1 USDC
// delivered, tx 0xbf56e6be...7133501).
//
...
export async function agentPay({ recipientAddress, amountUsdc }) {
  const apiKey = process.env.CIRCLE_API_KEY;
  const entitySecret = process.env.CIRCLE_ENTITY_SECRET;
  const owner = process.env.AGENT_WALLET_ADDRESS;   // SCA depositor (holds balance)
