// research-start.mjs — kick off an async analysis and hand back a job id.
//
// The analyst runs in research-background.mjs, which can take minutes
...
export async function handler(event) {
  ...
  if (typeof question !== "string" || question.trim() === "") {
    return json(400, { error: "Provide a non-empty string 'question'" });
  }
  ...
  // draft deploy resolve to a DIFFERENT deploy that has no research-
  ...
  try {
    const res = await fetch(`${base}/.netlify/functions/research-background`, {
      ...
      body: JSON.stringify({ question: question.trim(), jobId }),
    });
