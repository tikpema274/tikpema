...
export async function readStateAtAnchor({ reader, vault, holder, anchor, cashOnly }) {
  ...
  try {
    const [withdrawFee, depositFee, decimals, shares, maxRedeem] = await Promise.all([
      read("withdrawFee"), read("depositFee"), read("decimals"), read("balanceOf", [holder]), read("maxRedeem", [holder]),
    ]);
    const probe = 10n ** decimals;
    const [gross, net] = await Promise.all([read("convertToAssets", [probe]), read("previewRedeem", [probe])]);

    const state = shares === 0n ? "full" : maxRedeem === 0n ? "blocked" : maxRedeem >= shares ? "full" : "partial";
    let payability;
    if (shares > 0n) {
    ...
    return {
      ...
      redemption: { state, positionShares: shares.toString() },
      payability,
    };
  } catch (e) {
