...
import { verifyDirectPayment, fetchReceipt, TRANSFER_TOPIC } from "../netlify/functions/_checkout-verify.mjs";
...
const receipt = (logs, status = "0x1") => ({ status, logs });
...
section("1 — the real Arc shape: TWO logs, only the USDC-emitted one counts");
{
  ...
  const v = verifyDirectPayment({ receipt: r, merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
...
section("2 — 🚨 the native mirror ALONE is not a payment");
{
  ...
  const w = verifyDirectPayment({ receipt: wrongEmitter, merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
...
{
  const to = verifyDirectPayment({ receipt: receipt([log(USDC, BUYER, OTHER, ONE_FIVE)]), merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
  check("wrong `to` (not the merchant) → not paid", !to.paid && /to the merchant/.test(to.reason), to.reason);
  const from = verifyDirectPayment({ receipt: receipt([log(USDC, OTHER, MERCHANT, ONE_FIVE)]), merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
  check("🚨 wrong `from` (someone else's transfer to the merchant) → not paid", !from.paid, from.reason);
  const short = verifyDirectPayment({ receipt: receipt([log(USDC, BUYER, MERCHANT, ONE_FIVE - 1n)]), merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
  ...
  const rev = verifyDirectPayment({ receipt: receipt([log(USDC, BUYER, MERCHANT, ONE_FIVE)], "0x0"), merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
  check("reverted transaction (status 0x0) → not paid even with a matching log", !rev.paid && /status/.test(rev.reason), rev.reason);
  const none = verifyDirectPayment({ receipt: null, merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
  check("no receipt → not paid, 'no receipt'", !none.paid && /no receipt/.test(none.reason));
  const empty = verifyDirectPayment({ receipt: receipt([]), merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
  check("no logs → not paid", !empty.paid);
  const badAmt = verifyDirectPayment({ receipt: receipt([log(USDC, BUYER, MERCHANT, ONE_FIVE)]), merchant: MERCHANT, amountUsdc: "abc", from: BUYER });
...
{
  const over = verifyDirectPayment({ receipt: receipt([log(USDC, BUYER, MERCHANT, ONE_FIVE + 7n)]), merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
  check("paying more than asked still pays the order", over.paid && over.value === ONE_FIVE + 7n);
  const cased = verifyDirectPayment({ receipt: receipt([log(USDC.toUpperCase().replace("0X", "0x"), BUYER.toUpperCase().replace("0X", "0x"), MERCHANT, ONE_FIVE)]), merchant: MERCHANT.toUpperCase().replace("0X", "0x"), amountUsdc: "1.5", from: BUYER });
  check("addresses compare case-insensitively", cased.paid);
  const two = verifyDirectPayment({ receipt: receipt([log(USDC, BUYER, MERCHANT, 1n), log(USDC, BUYER, MERCHANT, ONE_FIVE)]), merchant: MERCHANT, amountUsdc: "1.5", from: BUYER });
