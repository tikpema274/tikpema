...
//   pin         a DELIBERATE in-code control that the PUBLISHED value did not change under a buyer
//               ("do not change" blocks in x402-quote / x402-vanilla-seller / _dd-x402). At the flip
//               these are the conscious acknowledgements, edited on purpose. Phase B decides their shape.
...
//               chain id beside AGENT_ID in dd-identity.mjs — deriving THAT from ARC.chainId would
//               make a mainnet flip silently claim identity 851891 lives on mainnet. It does not.
...
//               deposit refusal window, so those land together in ONE deploy — phase B. Listed here
//               so the flip to refuse cannot happen while one remains.
//
...
//   C4  every `phase-b` path IS on the DD surface (parsed from stamp-build.mjs, not restated), and no
//       unallowlisted hit is silently a DD-surface file — each is tagged so the reader knows the cost.
//
// ═══ MODE ═══════════════════════════════════════════════════════════════════════════════════
//   "warn"    (phase A) — unallowlisted hits and phase-b entries are REPORTED and the run exits 0.
//   "refuse"  (phase B) — any unallowlisted hit exits 1, AND a non-empty PHASE_B list exits 1: the flip
//             is legal only once every DD-surface re-literal has been fixed in the phase-B deploy.
//   The flip is the ONE constant below, changed in the same commit as the phase-B DD-surface edits.
import { readFileSync, existsSync } from "node:fs";
import { execFileSync } from "node:child_process";
import { ARC, } from "../netlify/functions/_arc.mjs";
import { GATEWAY } from "../netlify/functions/_gateway.mjs";

const MODE = "warn"; // ⛔ phase A. Flip to "refuse" in the phase-B commit — see the header.
...
const ALLOW = {
  // pins — deliberate "the published value did not change" controls in code (phase B decides their shape)
  // ⭐ `expect` pins the EXACT hit count per lever for these classes — a blanket allow on a file would hide a
  // NEW re-literal beside the pin (x402-quote held two re-literals beside its pin until phase A).
  "netlify/functions/x402-quote.mjs":          { cls: "pin", expect: { chainId: 1 }, why: "startup pin: published network/asset must not change under a buyer; twin of the _dd-x402 block" },
  "netlify/functions/x402-vanilla-seller.mjs": { cls: "pin", expect: { chainId: 1 }, why: "startup pin, third copy — see its own comment" },
  // records in code
...
const PHASE_B = {
  "netlify/functions/dd-analyze.mjs":          "ARC_RPC literal → ARC.rpc",
  "netlify/functions/_dd-x402.mjs":            "DD_VERIFYING_CONTRACT literal → GATEWAY.WALLET; startup pin twin of x402-quote (decide the pins' shape together)",
  "netlify/functions/_x402-confirm.mjs":       "GATEWAY_WALLET literal → GATEWAY.WALLET",
  "shared/onchain-analyze/endpoints.mjs":      "the quorum endpoint list names the public RPC host — decide: derive from dd-core's chains.mjs, or keep as an independent-provider list",
  "shared/dd/attest-circle.mjs":               "chainId beside the attestation identity — decide: record (like dd-identity) or derive",
};
...
const classify = (p) => {
  ...
  if (PHASE_B[p]) return { cls: "phase-b", why: PHASE_B[p] };
