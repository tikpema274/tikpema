...
export const UNWIRED_OK = {
  ...
  "test:feemanagerlive": "network-dependent (eth_calls Arc's PUBLIC RPC, recorded as throttled and observed ETIMEDOUT mid-run). Same call as gate:pins: a flaky network inside a BLOCKING aggregate manufactures tolerated red. ⭐ Its offline half — verify-fee-reconcile §9 — IS in test:all and asserts that the FeeManager/TMWF addresses are pinned as constants, that the `feeManager()` getter check which justified the literal is recorded beside it, and that the failure direction is written down. What only THIS can see is the one thing no offline check ever could: that the live contract still names those addresses. ⚠️ A stale literal degrades to `unreadable`, never to a false MATCHED — which is why the literal is safe, and is NOT why it would be acceptable to leave wrong. Run it when the upfront-fee migration lands, and periodically after.",
