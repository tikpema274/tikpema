...
section("3 — ⚠️ THE GAP: signed, then the tab closed before promote");
{
  ...
  console.log("        Closing it needs a reconcile that scans Arc for burns FROM the owner to the");
    console.log("        BridgingKit after the intent's block. ⚠️ This borrowed the receipt module's");
    console.log("        phrase 'SEPARATE, UNBUILT job' — which named the PROVISIONAL reconcile, and");
    console.log("        that is now BUILT (bridge-mint-sweep.mjs:94). THIS gap is a different job —");
    console.log("        scanning Arc for user-signed burns — and it is still unbuilt.");
