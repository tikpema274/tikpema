...
minutes. Both failures had been invisible, and one of them **invalidates what I recorded two entries
ago**.

## ⚠️ CORRECTION: `test:all` WAS NOT RED FOR 21 HOURS BECAUSE OF `test:research`

That entry named `test:research` (7th of 18) as the blocker and counted 11 masked suites. **Wrong,
and understated in both dimensions.**

🚨 **`test:probe` IS FIRST, AND IT HAS BEEN RED SINCE 2026-08-18 13:13** — `a306f70`, the first
deploy-record commit after the assertion landed at `78ab33e` (2026-08-14). So `test:all` never
reached step 2. **All 17 suites were masked, for ~2 days**, and `test:research`'s own 21 hours of
redness sat *inside* that shadow, unreachable and therefore undiscovered.

⭐⭐ **I FOUND THE SECOND-DEEPEST FAILURE AND CALLED IT THE CAUSE.** Diagnosing a red aggregate by
reading the failure it prints finds the FIRST one only — and a chain prints exactly one. The
correction did not come from thinking harder; it came from making every suite run.

## THE TWO FAILURES

**1. `test:probe` — `the COMMITTED stamp is null`.** The guard's own failure text is the diagnosis:
*"if this fails, someone committed a stamped value and prod can now report a STALE commit."*
`build-stamp.generated.mjs` says on its first line **DO NOT COMMIT A NON-NULL VALUE**, and every
`chore(dd): … deploy` record commit since 2026-08-18 did — three of them mine, today. Fixed by
`npm run stamp:clear` + committing the null. `test:probe` 70/1 → **71/0**.

**2. `test:dd` — `the module holding it imports NO billing code`.** Pre-existing and independent:
it fails with this session's work stashed. `_dd-rungs.mjs:38` has imported `_dd-x402.mjs` since
`e8cb0b9` (2026-08-19 09:12) — **~30 hours red, and completely invisible**, since it sits behind
`test:probe`. ⚠️ NOT YET JUDGED: the import pulls `resolvePayTo, ddPaymentRequirements,
DD_PRICE_HUMAN` — payment TERMS for building a 402, not a settlement OUTCOME. So either the guard is
now over-broad, or the "settlement is not in scope, at all" property genuinely regressed. **That is a
design call, not a lint fix, and it is left open deliberately.**

## 🚨🚨 AND THE STAMP DEFECT COST A REAL VERIFICATION — DEPLOY `6a86eead` IS NOT VERIFIED

Mid-bundle I ran `git checkout -- shared/build-stamp.generated.mjs` to drop a stamp regenerated by a
local build. Because the committed value was a real SHA rather than `null`, that restored a
**stale-but-plausible** stamp under a running deploy, and the bundler shipped it.

| | |
|---|---|
| deploy `6a86eead` reports | commit `759cdc4`, tree `539679416c29`, stamped `10:45:41` |
| that is | **the PREVIOUS deploy's identity** |
| local truth | commit `d1e6e1a`, tree `1b55863d67f9` |

⭐⭐ **AND `gate:deployed` SAID "✅ DEPLOY VERIFIED".** It compares the local stamp to the served
stamp — and both were the same stale bytes, because the local one had been reverted to exactly what
was deployed. **THE CHECK PASSED BY COMPARING TWO COPIES OF ONE STALE VALUE.** `stamp-build.mjs`'s
own header already records this shape from `0d16bfc`/`dd16f23`; it arrived again through a different
door — not a hash too narrow, but a stamp file reverted under the build.

⭐ Re-run afterwards, the gate correctly FAILS: *"local tree 1b55863d67f9 but production serves
539679416c29 … A deploy finished — it just was not this one."*

⚠️ **SO THE RATE INJECTION CANNOT BE CALLED LIVE.** Its files were unchanged on disk throughout and
were almost certainly bundled — but the instrument that would prove it is the one I corrupted, and
"almost certainly" is not a measurement. A clean re-deploy with nothing touching the stamp is the
only honest way to close it.

## ⭐ WHAT THE ROLL-UP CHANGED, CONCRETELY

* `NOT RUN 0` on every line of the summary — the number that did not exist before, and the one that
  would have made two days of masking obvious on day one.
* Two failures surfaced in **1.8 minutes** that had survived 2 days and 30 hours respectively.
* ⭐ The 15 suites after `test:probe` got their first honest reading since 2026-08-18, and **13 of
  them were fine** — which is itself worth knowing, because "red for two days" invites the assumption
  that everything behind it had rotted.
