...
| **env-assert has NO mainnet chainId / rpcHost rows — BY DESIGN** (fail-closed: a full migration boots to UNKNOWN and REFUSES until both are added from a published source). gatewayWallet.mainnet (`0x7777…00eE`) and gatewayHost.mainnet are already present. | Circle → code | Circle's published Arc mainnet chain id + RPC host added to `_env-assert.mjs` with the source URL in the comment; `test:envassert` green; a deliberate partial flip still produces the SPLIT refusal. | **FILLED IN CODE 2026-09-20** (deploy pending — `_env-assert.mjs` is on the DD surface, so the deploy that carries it rotates ddTree: one window): `chainId.mainnet 5042`, `rpcHost.mainnet rpc.mainnet.arc.io`, each citing docs.arc.io/arc/references/rpc-endpoints (read 2026-09-20; `eth_chainId` → 0x13b2 on that host). `verify-env-assert.mjs` now asserts: the rows are the published values; a FULL mainnet config classifies `mainnet`; a half-migration (any 1–3 levers moved) is a SPLIT and refuses naming both sides; an unpublished value is UNKNOWN and refuses. SETTLED when the deploy is verified. → `netlify/functions/_env-assert.mjs` header. |
